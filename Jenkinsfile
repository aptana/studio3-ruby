	...
	try {
		...
		timestamps() {
			stage('Checkout') {
				...
				minimums = getCoverageMinimums("Studio/studio3-ruby/${targetBranch}", isPR)
			}
...
"""
				timeout(10) {
					sh 'ant -propertyfile override.properties -f builders/com.aptana.radrails.build/build.xml build'
				}
...
studio3.p2.repo=file://${env.WORKSPACE}/studio3-core/dist/
studio3.test.p2.repo=file://${env.WORKSPACE}/studio3-core/dist-tests/
radrails.p2.repo=file://${env.WORKSPACE}/dist/
"""
						timeout(10) {
							sh 'ant -propertyfile test-override.properties -f builders/com.aptana.radrails.tests.build/build.xml build test'
						} // end timeout
					} // end mask passwords
				} // end xvnc
