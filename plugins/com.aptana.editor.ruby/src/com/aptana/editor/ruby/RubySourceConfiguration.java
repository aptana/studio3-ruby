...
public class RubySourceConfiguration implements IPartitioningConfiguration, ISourceViewerConfiguration {
	...
	public static final String[] CONTENT_TYPES = new String[] { DEFAULT, SINGLE_LINE_COMMENT, MULTI_LINE_COMMENT, REGULAR_EXPRESSION, COMMAND, STRING_SINGLE, STRING_DOUBLE };

	private static final String[][] TOP_CONTENT_TYPES = new String[][] { { IRubyConstants.CONTENT_TYPE_RUBY } };

	private final IPredicateRule[] partitioningRules = new IPredicateRule[] { new PartitionerSwitchingIgnoreRule(new EndOfLineRule("#", new Token(SINGLE_LINE_COMMENT))), //$NON-NLS-1$
	...
	static {
		...
		c.addTranslation(new QualifiedContentType(IRubyConstants.CONTENT_TYPE_RUBY), new QualifiedContentType("source.ruby.rails")); //$NON-NLS-1$ // FIXME Should just be source.ruby! Rails bundle should contribute the more specific scope
		...
		c.addTranslation(new QualifiedContentType(SINGLE_LINE_COMMENT), new QualifiedContentType(IRubyConstants.LINE_COMMENT_SCOPE));
		c.addTranslation(new QualifiedContentType(MULTI_LINE_COMMENT), new QualifiedContentType(IRubyConstants.BLOCK_COMMENT_SCOPE));
		c.addTranslation(new QualifiedContentType(REGULAR_EXPRESSION), new QualifiedContentType("string.regexp.classic.ruby")); //$NON-NLS-1$
